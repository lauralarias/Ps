...
const Intro = () => {
  ...
  const [showTmsTab, setShowTmsTab] = useState(true);
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden -mt-[80px]">
          ...
          <div className={`text-white space-y-8 transition-all duration-1000 pt-12 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            ...
            <div className="flex items-center space-x-8 pt-12">
              <div className="flex items-center space-x-3">
                ...
                <span className="text-blue-300 text-sm font-medium">FDA Approved</span>
              </div>
              <div className="flex items-center space-x-3">
                <div className="w-8 h-8 bg-blue-500/20 rounded-full flex items-center justify-center">
                  <svg className="w-4 h-4 text-blue-400" fill="currentColor" viewBox="0 0 20 20">
      ...
      {/* Floating TMS Info Tab */}
      {showTmsTab && (
        <div className="fixed bottom-8 right-8 z-50 bg-blue-100 rounded-xl shadow-lg px-6 py-5 flex flex-col items-center min-w-[270px] max-w-xs" style={{fontFamily: 'Comic Sans MS, Comic Sans, cursive'}}>
          <button
            className="absolute top-2 right-3 text-blue-400 text-2xl font-bold hover:text-blue-600 focus:outline-none"
            onClick={() => setShowTmsTab(false)}
            aria-label="Close"
          >
            &times;
          </button>
          <div className="text-blue-900 text-2xl text-center font-semibold mb-2 leading-snug">
            Have you tried 2+<br/>antidepressants?
          </div>
          <div className="text-blue-900 text-base text-center mb-4">
            Read more about how TMS can help you!
          </div>
          <button
            className="w-full bg-white text-blue-700 font-medium py-2 rounded-md text-lg hover:bg-blue-50 transition"
